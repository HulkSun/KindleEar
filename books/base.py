...
class BaseComicBook(BaseFeedBook):
        ...
        for item in self.feeds:
            ...
            if next_chapter_index < len(chapterList):
                ...
                for img in imgList:
                    page_count += 1
                    urls.append(
                        (chapter_title, "{}".format(page_count), img, None))
    ...
    def Items(self):
        ...
        decoder = AutoDecoder(isfeed=False)
        prevSection = ''
        min_width, min_height = self.min_image_size if self.min_image_size else (
            0, 0)
        ...
        for section, fTitle, url, desc in urls:
            ...
            if content == None:
                self.log.warn("Image adjust error, try again: {}".format(
                    url.encode('utf-8')))
                content = self.adjustImgContent(content)
                if content == None:
                    self.log.warn("Image adjust error: {}".format(
                        url.encode('utf-8')))
                    continue
